  ...
  return (
    <div className="dash-container">
      ...
      <main className="dash-main">
        ...
        {/* Google Business Profile OAuth & Automation Card */}
        <div className="dash-card">
          <h3 className="dash-card-title" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <svg style={{ width: '18px', height: '18px', fill: '#4285F4' }} viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z" fill="#EA4335"/></svg>
            Google Business Integration (Auto-Reply)
          </h3>
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginBottom: '1.25rem', lineHeight: 1.5 }}>
            Connect your official Google Business Profile to retrieve customer reviews in real-time and enable the AI auto-reply feature.
          </p>
          ...
          {client.google_account_id ? (
            <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem', alignItems: 'center' }}>
                <div>
                  <p style={{ margin: 0, fontSize: '0.85rem', color: '#64748b' }}>Connected Google Account</p>
                  <p style={{ margin: '2px 0 10px', fontSize: '0.95rem', fontWeight: 600, color: '#1e293b' }}>{client.google_account_id}</p>
                  
                  <p style={{ margin: 0, fontSize: '0.85rem', color: '#64748b' }}>Active Location ID</p>
                  <p style={{ margin: '2px 0 0', fontSize: '0.95rem', fontWeight: 600, color: '#1e293b' }}>{client.google_location_id || 'Unknown Location Profile'}</p>
                </div>
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <button
                    ...
                    onClick={async () => {
                      ...
                      setErrorMsg('');
                      try {
                        const res = await apiFetch(`/api/reviews/sync-and-reply`, {
                          method: 'POST',
                          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
                          body: JSON.stringify({ clientId: activeClientId })
                        });
                        const resData = await res.json();
                        if (!res.ok) throw new Error(resData.message || 'Auto-reply sync failed.');
                        setSuccessMsg(`Sync complete! Processed ${resData.processedReviews?.length || 0} reviews.`);
                      } catch (err: any) {
                        setErrorMsg(err.message || 'Verification update request failed.');
                      }
                    }}
                  >
                    Sync Reviews & Reply
