...
export default function LandingPage() {
  ...
  const howItWorks = [
    { step: '01', title: 'Share Your Link', desc: 'Generate a branded review funnel link and share it via WhatsApp, QR code, email, or any channel.' },
    { step: '02', title: 'Customer Rates You', desc: 'Customers see a clean Google-style interface and tap their star rating — no app download needed.' },
    { step: '03', title: 'Smart Routing', desc: '4+ stars go straight to Google with an AI-generated draft ready to paste. 3 and below stay private.' },
    { step: '04', title: 'Track Everything', desc: 'Monitor all feedback, analyse trends, and manage your configuration from the dashboard in real time.' },
  ];

  const testimonials = [
    { name: 'Ravi Sharma', role: 'Restaurant Owner, Bengaluru', quote: 'Within 3 weeks we went from 4.1 to 4.7 stars on Google. The diversion feature alone saved us from several bad reviews going public.', avatar: 'RS' },
    { name: 'Priya Nair', role: 'Salon Manager, Kochi', quote: 'Extremely easy to use. We share the QR code at checkout and customers love the smooth experience. Our review count tripled.', avatar: 'PN' },
    { name: 'Arun Menon', role: 'Clinic Director, Chennai', quote: 'The AI suggestions are brilliant. Patients who would normally not know what to write now leave detailed, genuine reviews.', avatar: 'AM' },
  ];
    ...
    <div style={{ fontFamily: "'Google Sans', Roboto, Arial, sans-serif", background: '#fff', color: '#202124', overflowX: 'hidden' }}>
      ...
      <nav style={{
        ...
        <div style={{ maxWidth: 1120, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 64 }}>
          {/* Logo — use relative paths, Vite base handles the prefix */}
          ...
          {/* Desktop nav links */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 32 }} className="ld-nav-links">
            {[['Features', '#features'], ['How it works', '#how-it-works'], ['Pricing', '#pricing']].map(([label, href]) => (
              <a key={label} href={href} style={{ fontSize: 14, color: '#5f6368', textDecoration: 'none', fontWeight: 500, transition: 'color 0.15s' }}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <button onClick={goToLogin} style={{ fontSize: 14, fontWeight: 500, color: '#467222', background: 'none', border: 'none', cursor: 'pointer', padding: '8px 16px', borderRadius: 100, transition: 'background 0.15s' }}
              onMouseEnter={e => (e.currentTarget.style.background = '#f0f4ef')}
              onMouseLeave={e => (e.currentTarget.style.background = 'none')}>
              Sign in
            </button>
            <button onClick={goToLogin} style={{ fontSize: 14, fontWeight: 500, color: '#fff', background: '#467222', border: 'none', cursor: 'pointer', padding: '9px 22px', borderRadius: 100, boxShadow: '0 1px 3px rgba(70,114,34,0.3)', transition: 'background 0.15s' }}
            ...
            {/* Mobile hamburger */}
            <button onClick={() => setMenuOpen(!menuOpen)} className="ld-hamburger"
        ...
        {menuOpen && (
          <div style={{ background: '#fff', borderTop: '1px solid #e8eaed', padding: '16px 24px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            {[['Features', '#features'], ['How it works', '#how-it-works'], ['Pricing', '#pricing']].map(([label, href]) => (
              <a key={label} href={href} onClick={() => setMenuOpen(false)} style={{ fontSize: 15, color: '#202124', textDecoration: 'none', fontWeight: 500 }}>{label}</a>
            ))}
            <button onClick={() => { goToLogin(); setMenuOpen(false); }} style={{ fontSize: 15, fontWeight: 600, color: '#fff', background: '#467222', border: 'none', cursor: 'pointer', padding: '12px', borderRadius: 100, marginTop: 8 }}>
              Sign in / Get started
            </button>
          </div>
        )}
      ...
      <section style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '120px 24px 80px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', maxWidth: 720, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 14, marginBottom: 28 }}>
            <img src="./image.png" alt="ReviewManager.in Reviews" style={{ height: 48, objectFit: 'contain' }} />
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16, flexWrap: 'wrap' }}>
            <button onClick={goToLogin}
              style={{ fontSize: 16, fontWeight: 600, color: '#fff', background: '#467222', border: 'none', cursor: 'pointer', padding: '14px 32px', borderRadius: 100, boxShadow: '0 2px 8px rgba(70,114,34,0.35)', transition: 'all 0.18s' }}
            ...
            <a href="mailto:contact@certifyied.com?subject=Demo%20Request"
              style={{ fontSize: 16, fontWeight: 500, color: '#467222', background: 'none', border: '1.5px solid #c5d9b5', padding: '13px 28px', borderRadius: 100, textDecoration: 'none', transition: 'all 0.18s', display: 'inline-block' }}
        ...
        {/* Dashboard mockup */}
        <div style={{ marginTop: 72, maxWidth: 860, width: '100%' }}>
          <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #e8eaed', boxShadow: '0 24px 64px rgba(0,0,0,0.1)', overflow: 'hidden' }}>
            {/* Browser chrome */}
            <div style={{ background: '#f8f9fa', padding: '12px 16px', borderBottom: '1px solid #e8eaed', display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div style={{ flex: 1, background: '#fff', borderRadius: 6, padding: '4px 12px', fontSize: 12, color: '#9aa0a6', border: '1px solid #e8eaed', maxWidth: 300, margin: '0 auto' }}>
                certifyied.com/reviews/dashboard
              </div>
            </div>
            <div style={{ padding: '28px 24px', background: '#f8fafc', display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 14 }}>
              {[
                { label: 'Overall Rating', value: '4.8 ★', color: '#fbbc05', sub: 'avg across all reviews' },
                { label: 'Total Submissions', value: '1,284', color: '#467222', sub: '+12% this month' },
                ...
                <div key={i} style={{ background: '#fff', borderRadius: 12, padding: '16px 18px', border: '1px solid #e8eaed' }}>
                  <p style={{ fontSize: 12, color: '#9aa0a6', margin: '0 0 6px', fontWeight: 500 }}>{card.label}</p>
                  <p style={{ fontSize: 24, fontWeight: 700, color: card.color, margin: '0 0 4px', letterSpacing: '-0.5px' }}>{card.value}</p>
        ...
        {/* Scroll cue */}
        <div style={{ marginTop: 40, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, color: '#9aa0a6' }}>
          <span style={{ fontSize: 13 }}>Explore features</span>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#9aa0a6" strokeWidth="2" strokeLinecap="round"><polyline points="6 9 12 15 18 9"/></svg>
        </div>
      </section>
      ...
      <section id="features" style={{ padding: '96px 24px', background: '#fff' }}>
        <div style={{ maxWidth: 1120, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 64 }}>
            ...
            <h2 style={{ fontSize: 'clamp(30px, 4vw, 44px)', fontWeight: 700, letterSpacing: '-1px', color: '#202124', margin: '0 0 16px' }}>Built for real reputation growth</h2>
            <p style={{ fontSize: 18, color: '#5f6368', maxWidth: 520, margin: '0 auto', lineHeight: 1.6 }}>Every feature designed to maximise positive reviews while keeping your brand protected.</p>
          </div>
          ...
          <div style={{ background: '#f8faf5', borderRadius: 20, padding: '48px', border: '1px solid #e0ecd6', display: 'flex', gap: 48, alignItems: 'center', flexWrap: 'wrap' }}>
            <div style={{ flex: '1 1 280px' }}>
              ...
              <h3 style={{ fontSize: 26, fontWeight: 700, color: '#202124', margin: '0 0 16px', letterSpacing: '-0.5px' }}>{features[activeFeature].title}</h3>
              <p style={{ fontSize: 17, color: '#5f6368', lineHeight: 1.7, margin: '0 0 28px' }}>{features[activeFeature].desc}</p>
              <button onClick={goToLogin} style={{ fontSize: 14, fontWeight: 600, color: '#467222', background: 'none', border: '1.5px solid #467222', cursor: 'pointer', padding: '10px 24px', borderRadius: 100, transition: 'all 0.15s' }}
            ...
            <div style={{ flex: '1 1 280px', background: '#fff', borderRadius: 16, padding: '32px', border: '1px solid #e0ecd6', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 200 }}>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 44, letterSpacing: 6, color: '#fbbc05', marginBottom: 16 }}>★★★★★</div>
                <p style={{ fontSize: 14, color: '#9aa0a6', maxWidth: 220, margin: '0 auto 16px', lineHeight: 1.6 }}>
                  "Excellent service! The team was super professional and efficient."
                </p>
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#1a73e8', color: '#fff', borderRadius: 100, padding: '8px 16px', fontSize: 13, fontWeight: 500 }}>
      ...
      <section id="how-it-works" style={{ padding: '96px 24px', background: '#f8f9fa' }}>
        <div style={{ maxWidth: 1120, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 64 }}>
            ...
            <h2 style={{ fontSize: 'clamp(30px, 4vw, 44px)', fontWeight: 700, letterSpacing: '-1px', color: '#202124', margin: '0 0 16px' }}>Up and running in minutes</h2>
            <p style={{ fontSize: 18, color: '#5f6368', maxWidth: 480, margin: '0 auto', lineHeight: 1.6 }}>No technical setup. No app install. Just a link that works.</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 28 }}>
            {howItWorks.map((step, i) => (
              <div key={i} style={{ background: '#fff', borderRadius: 16, padding: '32px 24px', border: '1px solid #e8eaed', transition: 'box-shadow 0.2s' }}
                ...
                <h3 style={{ fontSize: 18, fontWeight: 700, color: '#202124', margin: '0 0 10px' }}>{step.title}</h3>
      ...
      <section style={{ padding: '96px 24px', background: '#fff' }}>
        <div style={{ maxWidth: 1120, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 64 }}>
            ...
            <h2 style={{ fontSize: 'clamp(30px, 4vw, 44px)', fontWeight: 700, letterSpacing: '-1px', color: '#202124', margin: 0 }}>Businesses that grew their reputation</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 28 }}>
            {testimonials.map((t, i) => (
              <div key={i} style={{ background: '#f8faf5', borderRadius: 16, padding: '32px', border: '1px solid #e0ecd6', display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                <p style={{ fontSize: 15, color: '#3c4043', lineHeight: 1.7, margin: 0, fontStyle: 'italic' }}>{t.quote}</p>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 'auto' }}>
                  <div style={{ width: 40, height: 40, borderRadius: 20, background: '#467222', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, color: '#fff', flexShrink: 0 }}>{t.avatar}</div>
                  <div>
      ...
      <section id="pricing" style={{ padding: '96px 24px', background: '#f8f9fa' }}>
        <div style={{ maxWidth: 860, margin: '0 auto', textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: 'clamp(30px, 4vw, 44px)', fontWeight: 700, letterSpacing: '-1px', color: '#202124', margin: '0 0 16px' }}>Simple, transparent pricing</h2>
          <p style={{ fontSize: 18, color: '#5f6368', marginBottom: 56, lineHeight: 1.6 }}>No hidden fees. Start free and scale as you grow.</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 24 }}>
            {[
              ...
              <div key={i} style={{ background: plan.primary ? '#467222' : '#fff', borderRadius: 20, padding: '36px 28px', border: plan.primary ? 'none' : '1px solid #e8eaed', boxShadow: plan.primary ? '0 8px 32px rgba(70,114,34,0.3)' : '0 1px 4px rgba(0,0,0,0.04)', textAlign: 'left', position: 'relative' }}>
                {plan.primary && <div style={{ position: 'absolute', top: -12, left: '50%', transform: 'translateX(-50%)', background: '#fbbc05', color: '#202124', fontSize: 11, fontWeight: 700, padding: '4px 12px', borderRadius: 100, letterSpacing: 1, whiteSpace: 'nowrap' }}>MOST POPULAR</div>}
                <p style={{ fontSize: 14, fontWeight: 600, color: plan.primary ? 'rgba(255,255,255,0.8)' : '#467222', margin: '0 0 8px' }}>{plan.name}</p>
                ...
                <button onClick={goToLogin} style={{ width: '100%', padding: '12px', borderRadius: 100, fontSize: 14, fontWeight: 600, cursor: 'pointer', border: 'none', background: plan.primary ? '#fff' : '#f0f4ef', color: '#467222', transition: 'all 0.18s' }}
      ...
      <section style={{ padding: '80px 24px', background: '#467222' }}>
        <div style={{ maxWidth: 640, margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
            <button onClick={goToLogin} style={{ fontSize: 16, fontWeight: 600, color: '#467222', background: '#fff', border: 'none', cursor: 'pointer', padding: '14px 32px', borderRadius: 100, boxShadow: '0 2px 12px rgba(0,0,0,0.15)', transition: 'all 0.18s' }}
            ...
            <a href="mailto:contact@certifyied.com?subject=Demo%20Request" style={{ fontSize: 16, fontWeight: 500, color: '#fff', border: '1.5px solid rgba(255,255,255,0.5)', padding: '13px 28px', borderRadius: 100, textDecoration: 'none', transition: 'all 0.18s', display: 'inline-block' }}
